...
Definition Zeval_op2 (b:bool) :  Op2 ->  Z -> Z -> Tauto.rtyp b:=
  if b as b0 return (Op2 -> Z -> Z -> Tauto.rtyp b0)
  then Zeval_pop2 else Zeval_bop2.


Lemma Zeval_op2_hold : forall b op q1 q2,
    Tauto.hold b (Zeval_op2 b op q1 q2) <-> Zeval_pop2 op q1 q2.
Proof.
  destruct b.
  simpl ; tauto.
...
Definition Zeval_formula (env : PolEnv Z) (b:bool) (f : Formula Z):=
  let (lhs, op, rhs) := f in
    (Zeval_op2 b op) (Zeval_expr env lhs) (Zeval_expr env rhs).

Definition Zeval_formula' :=
  eval_formula  Z.add Z.mul Z.sub Z.opp (@eq Z) Z.le Z.lt (fun x => x) (fun x => x) (pow_N 1 Z.mul).

Lemma Zeval_formula_compat : forall env b f, Tauto.hold b (Zeval_formula env b f) <-> Zeval_formula env true f.
Proof.
  destruct b ; simpl.
  - tauto.
  - destruct f ; simpl.
    rewrite <- Zeval_op2_hold with (b:=false).
    simpl. tauto.
Qed.

Lemma Zeval_formula_compat' : forall env f,  Zeval_formula env true f <-> Zeval_formula' env f.
Proof.
  intros.
...
Lemma xnnormalise_correct :
  forall env f,
    eval_nformula env (xnnormalise f) <-> Zeval_formula env true f.
Proof.
  intros.
...
Lemma normalise_correct : forall (T: Type) env t (tg:T), eval_cnf eval_nformula env (normalise t tg) <-> Zeval_formula env true t.
Proof.
  intros.
...
Lemma negate_correct : forall T env t (tg:T), eval_cnf eval_nformula env (negate t tg) <-> ~ Zeval_formula env true t.
Proof.
  intros.
  rewrite <- xnnormalise_correct.
  unfold negate.
...
Definition cnfZ (Annot: Type) (TX : bool -> Type)  (AF : Type) (b:bool) (f : TFormula (Formula Z) Annot TX AF b) :=
  rxcnf Zunsat Zdeduce normalise negate true f.

Definition ZweakTautoChecker (w: list ZWitness) (f : BFormula (Formula Z) true) : bool :=
...
(*
Lemma agree_env_add : forall x s env env',
    agree_env (Vars.add x s) env env' ->
    agree_env s env env' /\ env x = env' x.
Proof.
  split.
  eapply agree_env_subset with (s1:=Vars.add x s); eauto.
  apply Vars.is_subset_add; auto.
  apply H.
  apply Vars.mem_add_same.
Qed.
*)
 ...
 *)
(*
Lemma is_subset_vars_of_pexpr :
  forall e v1 v2,
    Vars.is_subset v1 v2 ->
    Vars.is_subset v1 (vars_of_pexpr e v2).
Proof.
  induction e ; simpl; intuition.
  eapply Vars.is_subset_trans ; eauto.
  apply Vars.is_subset_add.
Qed.


Lemma is_subset_vars_of_bformula :
  forall b (f: @GFormula (Formula Z) bProp unit unit b) v1 v2,
    Vars.is_subset v1 v2 ->
    Vars.is_subset v1 (vars_of_bformula b f v2).
Proof.
  induction f ; simpl; intuition.
  destruct t; simpl.
  apply is_subset_vars_of_pexpr.
  apply is_subset_vars_of_pexpr; auto.
Qed.
*)
...
Definition ZTautoChecker  (f : BFormula (Formula Z) true) (w: list ZArithProof): bool :=
