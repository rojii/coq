...
let bound_monomials (sys: WithProof.t list) =
  let l = extract_all (fun ((p,o),_) -> match LinPoly.get_bound p with
                                        | None -> None
                                        | Some Vect.Bound.{cst;var;coeff} ->
                                           Some (Monomial.degree (LinPoly.MonT.retrieve var))) sys in
  let deg  = List.fold_left (fun acc ((p,o),_) -> max acc (LinPoly.degree p)) 0 sys in

  let vars = List.fold_left (fun acc ((p,o),_) -> ISet.union (LinPoly.monomials p) acc) ISet.empty sys  in

  let bounds = saturate_bin (fun (i1,w1) (i2,w2) ->
                   if i1 + i2 > deg then None
                   else match WithProof.mul_bound w1 w2 with
                        | None -> None
                        | Some b -> Some (i1 + i2,b)
                 ) (fst l) in
  let has_mon (_,((p,o),_)) =
    ...
    | None -> false
    | Some Vect.Bound.{cst;var;coeff} -> ISet.mem var vars in
...
let lia (can_enum:bool) (prfdepth:int) sys =
  ...
  let sys = bnd@(saturate_by_linear_equalities sys) in
